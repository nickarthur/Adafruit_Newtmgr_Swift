...
//TODO: - REMOVE THE FOLLOWING CODE NANTODO
         peripheralCell.onConnect = { [unowned self] in
         self.showPeripheralDetails()
         }
//MARK - END NANDEBUG END TODO:
        
        // Detail Subview
//MARK: BEGIN NANDEBUG
        //let isDetailViewOpen = false //row == tableRowOpen
        let isDetailViewOpen = true //row == tableRowOpen
//MARK - END NANDEBUG
        peripheralCell.baseStackView.subviews[1].isHidden = !isDetailViewOpen
        if isDetailViewOpen {
             setupPeripheralExtendedView(peripheralCell, advertisementData: peripheral.advertisement)
        }
        
        return peripheralCell
    }
}
// MARK: - BEGIN NAN DEBUG
func setupPeripheralExtendedView(_ peripheralCell:PeripheralTableViewCell, advertisementData: BlePeripheral.Advertisement) {
    
    
    peripheralCell.connectableValueLabel.text = (advertisementData.isConnectable?.description)
    peripheralCell.localNameValueLabel.text = advertisementData.localName ?? "<Unknown>"
    peripheralCell.manufacturerValueLabel.text = advertisementData.manufacturerString ?? "<Unknown>"
    peripheralCell.txPowerLevelValueLabel.text = "\(advertisementData.txPower) dbm" ?? "<Unknown>"
    peripheralCell.connectableValueLabel.text = advertisementData.isConnectable?.description ?? "<Unknown>"
